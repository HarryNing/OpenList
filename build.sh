...
if [ "$1" = "dev" ]; then
  ...
  webVersion=$(eval "curl -fsSL --max-time 30 $githubAuthArgs \"https://api.github.com/repos/$frontendRepo/releases/latest\"" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
fi
...
FetchWebRolling() {
  pre_release_json=$(eval "curl -fsSL --max-time 30 $githubAuthArgs -H \"Accept: application/vnd.github.v3+json\" \"https://api.github.com/repos/$frontendRepo/releases/tags/rolling\"")
...
FetchWebRelease() {
  release_json=$(eval "curl -fsSL --max-time 30 $githubAuthArgs -H \"Accept: application/vnd.github.v3+json\" \"https://api.github.com/repos/$frontendRepo/releases/latest\"")
...
BuildReleaseFreeBSD() {
  ...
  freebsd_version=$(eval "curl -fsSL --max-time 30 $githubAuthArgs \"https://api.github.com/repos/freebsd/freebsd-src/tags\"" | \
    jq -r '.[].name' | \
    grep '^release/14\.' | \
    grep -v -- '-p[0-9]*$' | \
    sort -V | \
