...
func CreateUser(c *gin.Context) {
	...
	req.Password = ""
	req.Authn = "[]"
	// Force set BasePath to isolate user
	userPath := "/users/" + req.Username
	req.BasePath = userPath

	if err := op.CreateUser(&req); err != nil {
		common.ErrorResp(c, err, 500, true)
	} else {
		// Auto create storage for the user is disabled for virtual-only usage.
		// Users will mount their own Cloud Drives via the API.
		// Physical path creation is also skipped as it refers to the ephemeral container fs if not mounted.
		log.Infof("user %s created, awaiting manual storage mount", req.Username)
