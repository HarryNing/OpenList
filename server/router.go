...
func Init(e *gin.Engine) {
	...
	_storage(auth.Group("/storage", middlewares.AuthNotGuest))
	admin(auth.Group("/admin", middlewares.AuthAdmin))
	_driver(auth.Group("/admin/driver", middlewares.AuthNotGuest))
	_storage(auth.Group("/admin/storage", middlewares.AuthNotGuest))
...
func _storage(g *gin.RouterGroup) {
	g.GET("/list", handles.ListStorages)
	g.GET("/get", handles.GetStorage)
	g.POST("/create", handles.CreateStorage)
	g.POST("/update", handles.UpdateStorage)
	g.POST("/delete", handles.DeleteStorage)
	g.POST("/enable", handles.EnableStorage)
	g.POST("/disable", handles.DisableStorage)
	// load_all remains admin only? or maybe safe? It reloads ALL storages. 
	// The specific user should probably not reload ALL. 
	// But let's keep it here for now or maybe keep load_all in admin? 
	// load_all affects global state. Let's keep load_all in admin and remove from here? 
	// The user request is "manage mount". load_all is operational.
	// Actually, CreateStorage automatically calls init. LoadAll is mostly for config file reload.
	// I will exclude load_all from user access for safety.
}

func _driver(g *gin.RouterGroup) {
	g.GET("/list", handles.ListDriverInfo)
	g.GET("/names", handles.ListDriverNames)
	g.GET("/info", handles.GetDriverInfo)
}
